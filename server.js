...
const CONFIG = {
    ...
    // Limit default untuk user baru
    defaultUserLimit: 3, // Setiap user dapat 3 limit per server
    mainServerLimit: 999 // Limit utama server (owner)
};
...
function checkLimit(username, server) {
    ...
    if (user.role === 'owner') {
        // Owner punya limit 999 (tidak terbatas)
        const limit = user.limits?.[server] || CONFIG.mainServerLimit;
    ...
    // User biasa punya limit 3
    const limit = user.limits?.[server] || CONFIG.defaultUserLimit;
    const used = user.used?.[server] || 0;
...
function useLimit(username, server) {
    ...
    const limit = user.role === 'owner' ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit;
    if (user.used[server] >= limit) return false;
...
function checkAllLimits(username) {
    const user = findUser(username);
    if (!user) return { allFull: false };
    
    const limit1 = user.role === 'owner' ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit;
    const limit2 = user.role === 'owner' ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit;
    
    const s1Used = user.used?.server1 || 0;
    const s2Used = user.used?.server2 || 0;
    
    const s1Full = s1Used >= limit1;
    const s2Full = s2Used >= limit2;
    const s1Off = !DB.settings.server1;
    const s2Off = !DB.settings.server2;
    
    return {
        allFull: (s1Full || s1Off) && (s2Full || s2Off),
        s1Full: s1Full || s1Off,
        s2Full: s2Full || s2Off,
        s1Off: s1Off,
        s2Off: s2Off,
...
app.post('/api/register', (req, res) => {
    try {
        ...
        // User baru dapat limit 3 per server
        DB.users.push({
            username: username,
            password: password,
            role: 'user',
            limits: { server1: CONFIG.defaultUserLimit, server2: CONFIG.defaultUserLimit },
            used: { server1: 0, server2: 0 }
        });
...
app.get('/api/user', (req, res) => {
    try {
        ...
        const limit1 = isOwner ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit;
        const limit2 = isOwner ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit;
        ...
        return res.json({
            ...
            user: {
                ...
                role: user.role,
                limits: { 
                    server1: user.limits?.server1 || limit1,
                    server2: user.limits?.server2 || limit2
                },
                used: user.used || { server1: 0, server2: 0 },
                isOwner: isOwner,
                maxLimit: isOwner ? CONFIG.mainServerLimit : CONFIG.defaultUserLimit
            },
            ...
            config: {
                ...
                mainServerLimit: CONFIG.mainServerLimit
            }
        });
    } catch (error) {
...
// BROADCAST READ
app.post('/api/broadcast/read', (req, res) => {
    try {
        if (!req.session || !req.session.user) {
            return res.json({ success: false });
        }
        ...
        if (broadcast) {
            ...
            if (!broadcast.readBy.includes(req.session.user.username)) {
                broadcast.readBy.push(req.session.user.username);
            }
        }
...
app.post('/api/redeem', (req, res) => {
    try {
        ...
        if (!server) {
            return res.json({ success: false, message: 'Pilih server terlebih dahulu!' });
        }
        ...
        // Tambah limit (max 999 untuk user biasa, tapi bisa lebih dari 3)
...
app.post('/api/owner/create-redeem', (req, res) => {
    try {
        ...
        return res.json({ success: true, code: code, limit: amount });
    } catch (error) {
        return res.json({ success: false, message: '❌ Terjadi kesalahan' });
    }
});
...
app.get('/api/owner/users', (req, res) => {
    try {
        ...
        const users = DB.users.map(u => ({
            username: u.username,
            role: u.role,
            limits: u.limits || { server1: CONFIG.defaultUserLimit, server2: CONFIG.defaultUserLimit },
...
// UPDATE USER LIMIT (OWNER)
app.post('/api/owner/update-limit', (req, res) => {
    try {
        if (!req.session || !req.session.user || req.session.user.role !== 'owner') {
            return res.json({ success: false, message: 'Unauthorized' });
        }
        
        const { username, server, limit } = req.body;
        const user = findUser(username);
        
        if (!user) return res.json({ success: false, message: 'User tidak ditemukan' });
        if (user.role === 'owner') return res.json({ success: false, message: 'Tidak bisa mengubah limit owner' });
        
        if (!user.limits) user.limits = { server1: CONFIG.defaultUserLimit, server2: CONFIG.defaultUserLimit };
        user.limits[server] = parseInt(limit) || CONFIG.defaultUserLimit;
        
        return res.json({ success: true, message: '✅ Limit berhasil diupdate' });
    } catch (error) {
        return res.json({ success: false, message: '❌ Terjadi kesalahan' });
    }
});

// RESET USER LIMIT (OWNER)
app.post('/api/owner/reset-limit', (req, res) => {
    try {
        if (!req.session || !req.session.user || req.session.user.role !== 'owner') {
            return res.json({ success: false, message: 'Unauthorized' });
        }
        ...
        user.used[server] = 0;
        return res.json({ success: true, message: '✅ Limit berhasil direset' });
    } catch (error) {
        return res.json({ success: false, message: '❌ Terjadi kesalahan' });
    }
});
...
app.post('/api/owner/delete-user', (req, res) => {
    try {
        ...
        if (DB.users[index].role === 'owner') return res.json({ success: false, message: 'Tidak bisa menghapus owner' });
...
app.post('/api/send', async (req, res) => {
    try {
        ...
        if (!DB.settings[server]) {
            return res.json({ 
                success: false, 
                message: `⚠️ ${server} sedang dimatikan oleh Owner. Silakan gunakan server lain.`
            });
        }
        ...
        if (!limitCheck.allowed) {
            return res.json({ 
                ...
                isOwner: limitCheck.isOwner || false
            });
        }
        
        // Kirim magic link ke API
        let result;
        if (server === 'server1') {
        ...
        if (result.status === 'success' || result.ok || result.message?.includes('berhasil')) {
            ...
            // Jika gagal, reset limit (free 1x)
        ...
        // Jika error, reset limit
        if (req.session?.user?.username) {
            resetLimit(req.session.user.username, req.body.server);
        }
        return res.json({ 
            success: false, 
...
app.post('/api/verify', async (req, res) => {
    try {
        ...
        if (req.session?.user?.username && req.session?.temp?.server) {
            resetLimit(req.session.user.username, req.session.temp.server);
        }
        return res.json({ 
            success: false, 
            message: `❌ Error: ${error.message}`,
...
app.listen(PORT, () => {
    ...
    console.log(`  📊 Default User Limit: ${CONFIG.defaultUserLimit} per server`);
    console.log(`  👑 Owner Limit: ${CONFIG.mainServerLimit} per server`);
    console.log('═══════════════════════════════════════════');
});